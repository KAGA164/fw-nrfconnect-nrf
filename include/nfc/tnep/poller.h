...
struct nfc_tnep_poller_msg {
	...
	/** TNEP NDEF Message contatins additional NDEF REcords
 ...
 *  writing. According to using different Tag Type,
 *  an API can be varoius for each of them.
 */
struct nfc_tnep_poller_ndef_api {
	 ...
	 *
	 * @param[out] ndef_buff Pointer to buffer where NDEF Message will be
	 ...
	 *
	 * @param[in] ndef_buff Buffer with NDEF Message to write. Buffer has to
 ...
 *
 * @param[in] tx_buff Pointer to TNEP Poller Tx buffer.
 * @param[in] cb Pointer to TNEP Poller callback structure.
 *
 * @retval 0 If the operation was successful.
 *           Otherwise, a (negative) error code is returned.
 */
int nfc_tnep_poller_init(const struct nfc_tnep_buff *tx_buff,
			 const struct nfc_tnep_poller_cb *cb);

/**@brief Set NDEF API fot the NFC TNEP Poller Device
 *
 * This function provides API for NDEF operation and indicates
 * the NFC Device Tag Type. API can be changed in runtime but
 * It must be set immediately after NFC Poller Device detect
 ...
 * the given NDEF Message. NDEF Message has to be stored until the all
 * operations on services will be finished.
 *
 ...
 *                    input. Count of found services as a output.
 *
 * @retval 0 If the operation was successful.
 *           Otherwise, a (negative) error code is returned.
 */
 ...
 * NFC Poller wait the specific amount of time to performs the NDEF Read
 * procedure to gets new data from the NFC Tag device. This operation
 * is asynchronous.
 *
 * @param[in] svc_buff Pointer to the buffer for new data from the NFC TNEP
 *                     Tag Device. Buffer must be stored until the select
 *                     procure will be finished.
 ...
 *
 * @param[in,out] svc_buff Pointer to buffer on the read data. It
 *                         must be kept until this operation will be
 *                         ended.
...
/**@brief Update the TNEP Service data.
 *
 * Function for updateing the TNEP Service data. The Service
 * must be selected first. After successful updating, library attempt to read
 * new Service data after the specific amount of time.
 * This operation is asynchronous.
 *
 * @param[in] msg Pointer to the NDEF Message which will be written.
 * @param[in] resp_data Pointer to received data. Buffer must be stored
 *                      until the update procure will be finished.
 * @param[in] resp_buff_size Size of the response buffer.
 *
 * @retval 0 If the operation was successful.
 *           Otherwise, a (negative) error code is returned.
 */
...
/**@brief Indicate NDEF data update.
 *
 * This function must be called immediately after NFC Reader/Writer
 * Device update NDEF message on NFC TNEP Tag Device.
 *
 * @retval 0 If the operation was successful.
 *           Otherwise, a (negative) error code is returned.
 */
int nfc_tnep_poller_on_ndef_update(void);

/**@brief Get the selected/active service.
 *
 * @retval Pointer to the selected/active service.
 *         If no slected/selected service then NULL pointer is returned.
 */
