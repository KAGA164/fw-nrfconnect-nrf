...
SHELL_IPC_DEFINE(shell_transport_ipc, CONFIG_SHELL_IPC_BACKEND_RX_RING_BUFFER_SIZE);
...
static void ipc_bound(void *priv)
{
	...
	LOG_DBG("Shell IPC endpoint bonded");
		...
		void *context)
{
	...
	LOG_DBG("Shell IPC initialization done");
...
const struct shell_transport_api shell_ipc_transport_api = {
	.init = init,
	.uninit = uninit,
	.enable = enable,
	.write = write,
...
static int enable_shell_ipc(const struct device *arg)
{
	ARG_UNUSED(arg);
        ...
        const struct device *dev = DEVICE_DT_GET(DT_NODELABEL(ipc0));
